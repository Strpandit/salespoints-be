module Reports
  module Admin
    class AllOrdersExporter < Reports::BaseExporter
      def generate
        range = date_range

        headers = ["Order #", "Channel", "Order Date", "Buyer Reference", "Seller Dealer", "Items Qty", "Subtotal (₹)", "Tax (₹)", "Total Amount (₹)", "Payment Mode", "Payment Status", "Order Status"]
        rows = []
        ...
        { title: "Admin All Orders Master Status Tracker", headers: headers, rows: rows }
      end
    end
  end
end
